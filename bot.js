...
const WEBSITE_DESIGNER_PROMPT = `Ты — креативный директор и архитектор сайтов студии Buildora AI.
По краткому описанию идеи клиента сделай следующее:

1. 💡 Предложи 2-3 разных варианта воплощения идеи (разные направления/подходы) — для каждого короткое название и 1-2 строки сути, чем вариант отличается от других.

2. 📝 Выбери наиболее сильный вариант (или объедини лучшее из нескольких) и на его основе напиши ПОЛНОЦЕННЫЙ ПРОФЕССИОНАЛЬНЫЙ ПРОМПТ для создания сайта. Промпт должен включать:
   - Структуру сайта — список разделов/страниц по порядку с описанием каждого
   - Концепцию дизайна — стиль, настроение, цветовая палитра (конкретные HEX-цвета), шрифты
   - Ключевые блоки главной страницы — что должно быть видно в первую очередь
   - Тональность текста и целевую аудиторию сайта

Отвечай на русском языке, используй понятные заголовки и списки. Пиши развёрнуто и профессионально, но без воды. Не пиши код — только текст вариантов и итоговый промпт.`;
...
function sendConceptAsDocument(chatId, concept) {
  ...
  return bot
    .sendDocument(
      chatId,
      filePath,
      { caption: '✅ Несколько идей и ваш профессиональный промпт для сайта готовы' },
    ...
    if (data === 'confirm_payment') {
      ...
      if (website && website.idea && website.concept) {
        bot
          .sendMessage(
            pending.chatId,
            '🌐 У вас уже есть подготовленный промпт для сайта — теперь можно его создать!',
            {
              reply_markup: {
                inline_keyboard: [
...
bot.on('message', (msg) => {
  ...
  if (state && state.waitingFor) {
    ...
    if (AI_ENABLED && state.waitingFor === 'create_website') {
      ...
      bot.sendMessage(
        chatId,
        `✅ Ваш запрос принят!${stillPendingText}\n\nХотите, чтобы наш ИИ разобрал вашу идею, предложил несколько вариантов и собрал профессиональный промпт для вашего сайта?`,
        {
          reply_markup: {
            inline_keyboard: [
